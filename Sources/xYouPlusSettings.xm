...
static const NSInteger xYouPlusSection = 500;

@interface YTSettingsSectionItemManager (xYouPlus)
- (void)updateTweakSectionWithEntry:(id)entry;
@end

extern NSBundle *xYouPlusBundle();
...
+ (NSArray *)settingsCategoryOrder {
    ...
    if (insertIndex != NSNotFound)
        [mutableOrder insertObject:@(xYouPlusSection) atIndex:insertIndex + 1];
    return mutableOrder;
}
%end
...
- (void)updateTweakSectionWithEntry:(id)entry {
    ...
    NSBundle *tweakBundle = xYouPlusBundle();
    ...
    YTSettingsSectionItem *version = [%c(YTSettingsSectionItem)
        ...
        selectBlock:^BOOL (YTSettingsCell *cell, NSUInteger arg1) {
            return [%c(YTUIUtils) openURL:[NSURL URLWithString:@"https://github.com/enokseth/xYouPlus/releases"]];
        }
    ];
    ...
    YTSettingsSectionItem *bug = [%c(YTSettingsSectionItem)
        ...
        detailTextBlock:nil
        selectBlock:^BOOL (YTSettingsCell *cell, NSUInteger arg1) {
            NSString *url = [NSString stringWithFormat:@"https://github.com/enokseth/xYouPlus/issues/new?assignees=&labels=bug&projects=&template=bug.yaml&title=[v%@] %@", VERSION_STRING, LOC(@"ADD_TITLE")];
    ...
    if ([settingsViewController respondsToSelector:@selector(setSectionItems:forCategory:title:icon:titleDescription:headerHidden:)])
        [settingsViewController setSectionItems:sectionItems forCategory:xYouPlusSection title:@"xYouPlus" icon:nil titleDescription:LOC(@"TITLE DESCRIPTION") headerHidden:YES];
    else
        [settingsViewController setSectionItems:sectionItems forCategory:xYouPlusSection title:@"xYouPlus" titleDescription:LOC(@"TITLE DESCRIPTION") headerHidden:YES];
}

- (void)updateSectionForCategory:(NSUInteger)category withEntry:(id)entry {
    if (category == xYouPlusSection) {
