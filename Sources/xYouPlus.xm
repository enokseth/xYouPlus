...
NSBundle *xYouPlusBundle() {
    static NSBundle *bundle = nil;
    static dispatch_once_t onceToken;
 	dispatch_once(&onceToken, ^{
        NSString *tweakBundlePath = [[NSBundle mainBundle] pathForResource:@"xYouPlus" ofType:@"bundle"];
        if (tweakBundlePath)
            bundle = [NSBundle bundleWithPath:tweakBundlePath];
        else
            bundle = [NSBundle bundleWithPath:ROOT_PATH_NS(@"/Library/Application Support/xYouPlus.bundle")];
    });
    return bundle;
}
...
// Hide "Play next in queue" - qnblackcat/xYouPlus#1138
...
%ctor {
    // Load xYou first so its functions are available for hooks.
    // dlopen([[NSString stringWithFormat:@"%@/Frameworks/xYou.dylib", [[NSBundle mainBundle] bundlePath]] UTF8String], RTLD_LAZY);
